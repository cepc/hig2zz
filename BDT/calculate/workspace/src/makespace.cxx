...
RooWorkspace *makespace(TString cname, int index, int lu)
{
    ...
    if (cname=="mzjv") poi.add(RooArgSet(mu_s, mu_tt, mu_ww, mu_zz, mu_zr, mu_bb, mu_cc, mu_gg));
    if (cname=="vzmj") poi.add(RooArgSet(mu_s, mu_tt, mu_ww, mu_zz));
    if (cname=="vzjm") poi.add(RooArgSet(mu_s, mu_tt, mu_ww, mu_zz, mu_bb, mu_cc));
    if (cname=="qzvm") poi.add(RooArgSet(mu_s, mu_tt, mu_ww, mu_zz, mu_bb, mu_cc));
    if (cname=="qzmv") poi.add(RooArgSet(mu_s, mu_tt, mu_ww, mu_zz, mu_zr, mu_bb, mu_cc, mu_mu));
...
RooAbsPdf* pdffit(TString cname, RooRealVar &var, RooDataSet &data, TString func)
{
    ...
    RooFitResult *result_mu;
        ...
        RooRealVar meanCB("meanCB","",125.,123.,126.);
        RooRealVar sigmaCB("sigmaCB","",0.5,0.,5.);
        RooRealVar alphaCBLo("alphaCBLo","",1.,0.,5.);
        RooRealVar alphaCBHi("alphaCBHi","",1.,0.,5.);
        RooRealVar nCBLo("nCBLo","",2.,0.,5.);
        RooRealVar nCBHi("nCBHi","",2.,0.,5.);
    ...
    if ( func.Contains("keys")  || func.Contains("binned") )
    {
        ...
        if (cname.Contains("imp")) 
              rho=2;        //qqtt不自然的样子 抹平
        if ((cname.Contains("qqqq")) ||(cname.Contains("vvqq")) || cname.Contains("yy") || (cname.Contains("w4q" ) ||(cname.Contains("vvhbb" ) )))
              rho=5;
