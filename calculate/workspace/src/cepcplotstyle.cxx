...
void WritePreliminary(){
	TLatex * prelim = new TLatex(0.50,0.92, "CEPC simulation");
	prelim->SetNDC();
...
void WriteChannel(char* channel){
	TLatex * prelim = new TLatex(0.50,0.86, channel);
	prelim->SetNDC();
	prelim->SetTextFont(22);
...
void PlotDataRooFit2(Char_t * figfilename, 
		RooAbsData* data, RooRealVar x, 
		std::vector<RooAbsPdf*> pdfList, 
		char * xtitle, char * ytitle, bool prelim, char* channel, double maxy, RooFitResult *res){
	...
	data->plotOn(xframe, Name("data"), MarkerStyle(21));
	for( unsigned int i =0; i<pdfList.size(); i++){
		if( i == 0)
		pdfList[0]->plotOn(xframe, Name("model"), Components(*pdfList[i]), LineStyle(linestyles[i]), LineWidth(3), LineColor(kColor[i+1]));
		else
	...
	RooArgList initPars  = res->floatParsInit();
        RooArgList finalPars = res->floatParsFinal();
	Int_t nFree = finalPars.getSize();
        Int_t nBins = xframe->GetNbinsX();
        Int_t ndf   = nBins-nFree;
        Double_t chi2 = xframe->chiSquare("model","data",nFree);
	Double_t prob = TMath::Prob(chi2,nFree);
	...
	//
	TLegend * leg = new TLegend(0.62, 0.79, 0.92, 0.94);
	//Char_t name[100];	
	TString name;	
	if(data)
	...
	for( unsigned int i =0; i<pdfList.size(); i++){
		...
		TString name(pdfList[i]->GetTitle());
		//if(i==0) leg->AddEntry(line, name, "LF");
		//else     leg->AddEntry(line, name, "LF");
		if(name.Contains("model_sig")) leg->AddEntry(line, "Signal", "LF");
		else if(name.Contains("model_bkg")) leg->AddEntry(line, "BKG", "LF");
		else if(name.Contains("model")) leg->AddEntry(line, "Signal+BKG", "LF");
	}

	char tempstr[100];
	sprintf(tempstr, "#chi^{2}=%.2f, Prob=%.2f", chi2*ndf, prob);
	TLatex * coeff = new TLatex(0.92,0.79, tempstr);
        coeff->SetNDC();
        coeff->SetTextFont(22);
        coeff->SetTextSize(0.035);
        coeff->SetTextAlign(33);
        coeff->Draw();
	...
	leg->Draw();
        ...
        RooPlot* chiframe = x.frame(Title("Residual Distribution"));
        chiframe->addPlotable(hresid,"P");
        c1->cd(2);
	gPad->SetPad(0,0,800,250);
        chiframe->Draw();*/
	...
	c1->Print(name); */
	TString ffilename(figfilename);
	TString temp = ffilename+".png";
	c1->Print(temp);
	temp = ffilename+".pdf";
        c1->Print(temp);
	temp = ffilename+".eps";
...
void PlotDataRooFit1(Char_t * figfilename, 
		RooAbsData* data, RooRealVar x, 
		std::vector<RooAbsPdf*> pdfList, 
	...
	//
	TLegend * leg = new TLegend(0.25, 0.65, 0.55, 0.80);
	//Char_t name[100];	
	TString name;	
	if(data)
		leg->AddEntry(data, "CEPC Simulation", "LEPF");
