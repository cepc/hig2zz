...
void ISOlatedLeptonFinderProcessor::processEvent( LCEvent * evt ) { 
  ...
  std::cout << "Isolepton: processEvent " << std::endl;
	_rpJetMap.clear();
	...
	for (int i = 0; i < npfo; i++ ) {
		...
		if ( IsISOlatedLepton( pfo ) ) {
			...
			//std::cout << "IsISOLatedLepton = yes" << std::endl;
		}
		else {
	...
	// For test 
	{
	  ...
	  int n3 = otIsoZLepCol->getNumberOfElements();
	  int n4 = otPFOsRemovedIsoZLepCol->getNumberOfElements();

	  std::cout << "N(IsoLepton) = "          << n1 << std::endl;
	  std::cout << "N(Without IsoLepton) = "  << n2 << std::endl;
	  std::cout << "N(ZIsoLepton) = "         << n3 << std::endl;
	  std::cout << "N(Without ZIsoLepton) = " << n4 << std::endl;

	  // Printout the collection names : temporary !!
	  std::cout << "ofPFPsRemovedIsoLepCol : " << _outputPFOsRemovedIsoLepCollection.c_str() << std::endl;
	  std::cout << "otIsoLepCol,           : " << _outputIsoLepCollection.c_str()  << std::endl;
...
void ISOlatedLeptonFinderProcessor::end() { 
  ...
  std::cout << "ISOlatedLeptonFinderProcessor::end()  " << std::endl;
}

vector<ReconstructedParticle*> ISOlatedLeptonFinderProcessor::getZLeptonPair( LCCollection* leps ){
...
double ISOlatedLeptonFinderProcessor::getConeEnergy( ReconstructedParticle* pfo ) {
...
void ISOlatedLeptonFinderProcessor::getCalEnergy( ReconstructedParticle* pfo , double* cale) {
	double ecal = 0;
	double hcal = 0;
